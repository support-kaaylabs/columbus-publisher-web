...
import { formatDate, subtractDates, getCurrentWeek, getPreviousWeek } from './date-helper';
...
const getDateType = (dateType: any) => {
  const dateObj:any = {
	  ...
	  thisWeek: () => getCurrentWeek({ unit: 'weeks', fmt: 'DD MMM', type: 'Week' }),
	  lastWeek: () => getPreviousWeek({ unit: 'weeks', fmt: 'DD MMM', type: 'Week' }),
...
export const getLayoutByType:React.FC<any> = (props) => {
  const { type, onChange = () => { }, value, autoCompleteOptions = {}, name='', label, filter = {}, data } = props;
  let layout =<></>;
  switch (type) {
	  case 'auto-complete':
    const { onSelect = () => { }, onSearch = () => { }, apiData, loading } = autoCompleteOptions;
    layout = (
        ...
        <Col>
            ...
            // value={value}
            style={{ width: 200 }}
            loading={_.get(loading, 'field') === name ? _.get(loading, 'loading') : false}
    ...
    layout = (
        ...
        <Col><label>{label}:</label></Col>
        <Col>
			  {!['customDate', 'customDateRange'].includes(value) && <Select
    ...
    layout = (
        ...
        <Col><label>{label}:</label></Col>
        <Col>
			  <InputNumber min={min} max={max} onChange={(e) => onChange(e, name)} name={name} />
        </Col>
		  </Col>
    );
    break;
  case 'drop':
		  const {  onClick = () => { }, eventData, reportId, eventLoading } = autoCompleteOptions;
    layout = (
		  <Col className="reports-filter" span={12}>
        <Col><label>{label}:</label></Col>
        <Col>
            ...
            allowClear
			 ...
			 {(eventData || []).map((item:any) => (
              <Select.Option key={item.key} value={item.event_name}>{item.event_name}</Select.Option>
            ))}
			  </Select>
        </Col>
		  </Col>
    ...
    layout = (
        ...
        <Col><label>{label}:</label></Col>
        <Col>
			  <Input onChange={(e) => onChange(e.target.value, name)} name={name} style={{ width: 200 }} />
        </Col>
		  </Col>
    );
    break;
